#!/usr/bin/env python3
"""
eval_scene_quality_mini_v4.py — Scene-quality ranking for Sentinel-2 L2A scenes (v4: CRS-aware + MGRS tile filter)
...
def _read_b04_window(
    ...
    try:
        with rasterio.open(href) as src:
            ...
            # Reproject the geographic centroid to the dataset's CRS (S2 COGs
            # are in UTM, not geographic).  This mirrors the logic in
            # src/s2_patch_extractor.py:_download_band_window.
            dst_crs = src.crs
            src_crs_epsg = dst_crs.to_epsg() if dst_crs is not None else None
            if src_crs_epsg is not None and src_crs_epsg != 4326 and _HAS_PYPROJ:
                xformer = _PyprojTransformer.from_crs(
                    "EPSG:4326", f"EPSG:{src_crs_epsg}", always_xy=True
                )
                cx, cy = xformer.transform(centre_lon, centre_lat)
            else:
                cx, cy = centre_lon, centre_lat
            try:
                px, py = src.index(cx, cy)
            except Exception:
                px, py = src.width // 2, src.height // 2

            # Window size in pixels.  Convert half_side_deg to metres via
            # the great-circle approximation (~111 km per degree) and then
            # to pixels using the dataset's transform.  This works whether
            # the dataset is in degrees (small transform) or metres (UTM).
            half_side_m = half_side_deg * 111_000.0
            try:
                m_per_px_x = abs(src.transform.a)
                m_per_px_y = abs(src.transform.e)
            except Exception:
                m_per_px_x = m_per_px_y = 10.0  # S2 10 m fallback
            if m_per_px_x <= 0:
                m_per_px_x = 10.0
            if m_per_px_y <= 0:
                m_per_px_y = 10.0
            half_px = max(1, int(half_side_m / max(m_per_px_x, m_per_px_y)))
...
def _parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--tile-filter", type=str, default="29SNB",
                   help="Restrict to STAC items whose ID contains this MGRS tile "
                        "(default: 29SNB = Algarve). Pass '' to disable.")
