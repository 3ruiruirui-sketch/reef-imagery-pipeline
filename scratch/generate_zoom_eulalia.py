...
def generate_zoom(buffer_m, out_name):
    print(f"Generating image with buffer={buffer_m}m...")
    ...
    temp_path = f"scratch_temp_{buffer_m}.png"
    ...
    if os.path.exists(temp_path):
        os.remove(temp_path)
    print(f"✅ Saved buffer={buffer_m}m to: {dst_path}")

# 1. Broad view (500m buffer = 1000m wide)
generate_zoom(500, "pedra_santa_eulalia_green_broad.png")

# 2. Detailed zoom (150m buffer = 300m wide - close up on reef crest!)
generate_zoom(150, "pedra_santa_eulalia_green_zoom.png")
