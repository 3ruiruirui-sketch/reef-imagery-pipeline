...
def evaluate_date_site(lat, lon, date_str, site_name, buffer_m=500):
    print(f"\n--- Searching {site_name} on {date_str} ---")
    ...
    if not items:
        print(f"  ❌ No Sentinel-2 scene found for {date_str}")
        return None
    ...
    print(f"  ✓ Local cloud cover at GPS: {local_cloud_pct:.1f}%")
    print(f"  ✓ SNR: {snr_mean:.1f} | Cleanliness (calmness): {fft_cleanliness:.0f} | Kd (clarity): {kd_est:.4f}")
    print(f"  ✓ BVI ML Score: {prediction['score']:.3f} ({prediction['mode']})")
    return prediction

target_dates = ["2025-09-25", "2025-10-02", "2023-09-01"]
for d in target_dates:
    evaluate_date_site(37.068978, -8.210328, d, "Pedra de Santa Eulalia")
    evaluate_date_site(37.058150, -8.209820, d, "Pedra do Alto")
