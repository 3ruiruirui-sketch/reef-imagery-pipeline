...
def fetch_depth(
    ...
    *,
    ...
    log.info(
        ...
        bbox, width, height, output_path,
    )

    resp = requests.get(_WCS_URL, params=params, timeout=timeout, stream=True)
    if resp.status_code != 200:
        raise RuntimeError(
            f"EMODnet WCS returned HTTP {resp.status_code}: {resp.text[:200]}"
        )
    ...
    log.info(
        ...
        output_path, height, width, valid_px,
...
def build_habitat_mask(
    ...
    *,
    ...
    log.info(
        "Habitat mask: reef=%d px (%.1f%%)  sand=%d px (%.1f%%)  "
        "outside_window=%d px  nodata=%d px",
        n_reef, 100 * n_reef / n_win if n_win else 0,
        n_sand, 100 * n_sand / n_win if n_win else 0,
