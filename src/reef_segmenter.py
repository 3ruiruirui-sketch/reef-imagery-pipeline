...
def normalize_depth(arr: np.ndarray,
                    min_depth_m: float = -SDB_OPTICAL_LIMIT_M,
                    max_depth_m: float = 0.0) -> np.ndarray:
...
def _load_model(weights_path: Optional[str] = None):
    ...
    if _MODEL is not None and _MODEL_KEY == path:
        return _MODEL
...
def segment_reef_array(depth: np.ndarray, threshold: float = 0.5,
                       tile: int = _TILE, weights_path: Optional[str] = None) -> dict:
    """
...
def segment_reef_tif(depth_tif: str, out_tif: Optional[str] = None,
                     threshold: float = 0.5, weights_path: Optional[str] = None) -> dict:
    """
